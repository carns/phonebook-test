        ...
        YP_provider_t* provider)
{
    ...
    if (phonebooks_array && json_object_is_type(phonebooks_array, json_type_array)) {
        for (size_t i = 0; i < json_object_array_length(phonebooks_array); ++i) {
            ...
            void* context = NULL;
            int ret = backend->create_phonebook(mid, p, json_object_to_json_string(phonebook_config), &context);
            if(ret != YP_SUCCESS) {
...
static void YP_create_phonebook_ult(hg_handle_t h)
{
    ...
    /* create the new phonebook's context */
    void* context = NULL;
    ret = backend->create_phonebook(provider->mid, provider, in.config, &context);
    if(ret != YP_SUCCESS) {
        out.ret = ret;
...
static void YP_open_phonebook_ult(hg_handle_t h)
{
    ...
    /* create the new phonebook's context */
    void* context = NULL;
    ret = backend->open_phonebook(provider->mid, provider, in.config, &context);
    if(ret != YP_SUCCESS) {
